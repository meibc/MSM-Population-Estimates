...
```{r}
nchscodes <- read_csv('/Users/meibinchen/Library/CloudStorage/OneDrive-JohnsHopkins/EEE HIV Stigma/EEE Data/Other/GSS and NCHS Data for Marginal Dist/NCHSurb_rural_codes.csv')
...
```
# Distribution of Males 18+ by Age Group by County
```{r}
# read in previously downloaded file for ACS Age-Specific Distributions 
acs_age_df <- read.csv("/Users/meibinchen/Library/CloudStorage/OneDrive-JohnsHopkins/EEE HIV Stigma/EEE Data/Other/ACS/ACSST5Y2023.S0101-Data.csv")
...
params_age_acs <- params_age_acs %>%
  ...
  mutate(GEOID = ifelse(County == "Petersburg Borough" & ST_ABBREV == "AK", "02195", GEOID)) %>%
  mutate(GEOID = ifelse(County == "LaSalle County" & ST_ABBREV == "IL", "17099", GEOID)) %>%
  mutate(GEOID = ifelse(County == "Doña Ana County" & ST_ABBREV == 'NM', "23027", GEOID)) %>%
  filter(!(is.na(GEOID)))
...
# Store to read in R
```{r}
save(params_gss, params_gss_age, params_gss_race, params_gss_income,
     params_gss_educ, params, nchscodes, params_age_acs, file = "pop_est_params.RData")
```