...
#   
#   # --- Males 18+ by Race ---
#   male_race_blk <- array(NA_real_, dim = c(n_race, nC, m))
#   for (a in 1:n_income) {
#     means_male_race <- params_acs_dem %>% 
#       filter(Category == "Race", factor_lev == races[a]) %>% pull(est)
#     sd_male_race <- params_acs_dem %>% 
#       filter(Category == "Race", factor_lev == races[a]) %>% pull(moe_se)
#     male_income_blk[a,,] <- matrix(
#       rnorm(nC * m, mean = means_male_race, sd = sd_male_race),
#       nrow = nC, ncol = m
#     )
#   }
#   
#   male_race_blk[male_race_blk < 0] = 0
#   
#   male_race_df <- as.data.frame.table(male_income_blk, responseName = "male_pop") %>%
#     rename(race = Var1, county_index = Var2, sim_index = Var3)
#   male_race_df <- male_race_df %>% mutate(
#     race = as.integer(race),
#     county_index = as.integer(county_index),
#     sim_index = as.integer(sim_index)
#   )
#   arrow::write_parquet(male_race_df, file.path("male_draws", paste0("adj_male_race_blk", chunk_id, ".parquet")))
#   
#   # --- Males 18+ by Ethnicity ---
#   # --- Males 18+ by Ethnicity ---
#   male_eth_blk <- array(NA_real_, dim = c(n_ethn, nC, m))
#   for (a in 1:n_eth) {
#     means_male_eth <- params_acs_dem %>% 
#       filter(Category == "Ethnicity", factor_lev == ethn[a]) %>% pull(est)
#     sd_male_eth <- params_acs_dem %>% 
#       filter(Category == "Ethnicity", factor_lev == ethn[a]) %>% pull(moe_se)
#     male_eth_blk[a,,] <- matrix(
#       rnorm(nC * m, mean = means_male_eth, sd = sd_male_eth),
#       nrow = nC, ncol = m
#     )
#   }
#   
#   male_eth_blk[male_eth_blk < 0] = 0
#   
#   male_eth_df <- as.data.frame.table(male_eth_blk, responseName = "male_pop") %>%
#     rename(ethnicity = Var1, county_index = Var2, sim_index = Var3) %>%
#     mutate(
...
#   arrow::write_parquet(
#     male_eth_df, 
#     file.path("male_draws", paste0("adj_male_eth_blk", chunk_id, ".parquet"))
#   )
#   
#   # Cleanup
#   rm(male_age_blk, male_age_df, male_blk, male_educ_blk, male_educ_df, male_income_blk, male_income_df, male_race_blk, male_race_df)
#   gc()
# }
```
