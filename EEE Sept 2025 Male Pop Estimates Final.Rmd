...
dem_config <- list(
  age = list(
    n       = n_age,
    levels  = ages,
    params  = params_gss_age,
    varname = "agecat"
  ),
  educ = list(
    n       = n_educ,
    levels  = educs,     # not educ_levels
    params  = params_gss_educ,
    varname = "educat"
  ),
  income = list(
    n       = n_income,
    levels  = incomes,   # not income_levels
    params  = params_gss_income,
    varname = "inccat"
  )
)
...
for (j in seq(1L, n_sim, by = n_chunk)) {
  ...
  male_age_df <- as.data.frame.table(male_age_blk, responseName = "male_pop") %>%
    rename(age_group = Var1, county_index = Var2, sim_index = Var3) %>%
    mutate(
  ...
  for (a in 1:n_educ) {
    means_male_educ <- params_acs_dem %>% filter(Category == "Education", factor_lev == educs[a]) %>% pull(est)
    sd_male_educ <- params_acs_dem %>% filter(Category == "Education", factor_lev == educs[a]) %>% pull(moe_se)
  ...
  male_educ_df <- as.data.frame.table(male_educ_blk, responseName = "male_pop") %>%
    rename(educ_group = Var1, county_index = Var2, sim_index = Var3)
  male_educ_df <- male_educ_df %>% mutate(
    educ_group = as.integer(educ_group),
    county_index = as.integer(county_index),
    sim_index = as.integer(sim_index)
  )
  ...
  for (a in 1:n_income) {
    means_male_income <- params_acs_dem %>% filter(Category == "Income", factor_lev == incomes[a]) %>% pull(est)
    sd_male_income <- params_acs_dem %>% filter(Category == "Income", factor_lev == incomes[a]) %>% pull(moe_se)
  ...
  male_income_df <- as.data.frame.table(male_income_blk, responseName = "male_pop") %>%
    rename(income_group = Var1, county_index = Var2, sim_index = Var3)
  ...
  for (j in seq(1L, n_sim, by = n_chunk)) {
    ...
    for (d in seq_len(n_dem)) {
      for (u in seq_len(n_urb)) {
        row <- params[params[[dem_var]] == levels[d] &
                        params$Urbanicity == urb_levels[u], ]
...
```{r}
# Make this into a function where you can specify which demographic you're interested in (e.g., age, education, income)

# dem: one of "age", "education", "income"
