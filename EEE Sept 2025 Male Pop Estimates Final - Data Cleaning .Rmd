...
params_income_acs <- params_income_acs %>%
  mutate(ST_ABBREV = ifelse(County == 'District of Columbia', 'DC', ST_ABBREV)) %>%
  left_join(nchscodes, by = c("County" = "CTYNAME", "ST_ABBREV" = "ST_ABBREV")) %>%
  mutate(GEOID = ifelse(County == "Petersburg Borough" & ST_ABBREV == "AK", "02195", GEOID)) %>%
  mutate(GEOID = ifelse(County == "LaSalle County" & ST_ABBREV == "IL", "17099", GEOID)) %>%
  mutate(GEOID = ifelse(County == "Doña Ana County" & ST_ABBREV == 'NM', "23027", GEOID)) %>% 
  filter(!ST_ABBREV %in% c('PR')) %>%
  filter(!is.na(GEOID)) %>%
  filter(!(ST_ABBREV == "CT" & str_detect(County, "County")))

params_edu_acs <- params_edu_acs %>%
  mutate(ST_ABBREV = ifelse(County == 'District of Columbia', 'DC', ST_ABBREV)) %>%
  left_join(nchscodes, by = c("County" = "CTYNAME", "ST_ABBREV" = "ST_ABBREV")) %>%
  mutate(GEOID = ifelse(County == "Petersburg Borough" & ST_ABBREV == "AK", "02195", GEOID)) %>%
  mutate(GEOID = ifelse(County == "LaSalle County" & ST_ABBREV == "IL", "17099", GEOID)) %>%
...
save(params_gss, params_gss_age, params_gss_race, params_gss_income,
     params_gss_educ, params, nchscodes, params_acs_dem, file = "pop_est_params.RData")
```