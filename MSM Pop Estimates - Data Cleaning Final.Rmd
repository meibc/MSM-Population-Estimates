...
#basepath <- "/Users/karenyu/Library/CloudStorage/OneDrive-SharedLibraries-JohnsHopkins/Meibin Chen - EEE Data/Other"
```

```{r}
# Convert ACS estimate + MOE(90%) -> lognormal (mu, sigma)
acs_to_lognorm <- function(estimate, moe, moe_level = 0.90) {
  if (is.na(estimate) || is.na(moe) || estimate <= 0) {
    return(tibble(mu = NA_real_, sigma = NA_real_))
  }
  z <- qnorm((1 + moe_level) / 2)  
  se <- moe / z
  v  <- se^2
  sigma2 <- log1p(v / (estimate^2)) 
  tibble(
    mu    = log(estimate) - 0.5 * sigma2,
    sigma = sqrt(sigma2)
  )
}
```
...
# Data Cleaning: Race
```{r}
race_tables <- c("A", "B", "C", "D", "E", "F", "G")
race_names  <- c("white", "black", "native", "asian", "pacific", "other", "two_or_more")

# 5-year data (2023), for all states except for CT
acs_race_2023 <- map2(race_tables, race_names, function(tbl, race) {
  get_acs(
    geography = "county",
    variables = c(paste0("B01001", tbl, "_002E"), paste0("B01001", tbl, "_002M")),
    year = 2023,
    survey = "acs5",
    cache_table = TRUE
  ) %>%
    mutate(race = race, year = 2023)
})

acs_race_df <- bind_rows(
  bind_rows(acs_race_2023)
)

acs_race_df <- acs_race_df %>%
  mutate(
    race = case_when(
...
cols <- c("estimate", "moe")
sub_race_df_clean <-acs_race_df %>%
  mutate(across(all_of(cols),as.numeric))

sub_race_df_clean <- sub_race_df_clean %>%
  separate(NAME, into = c("County", "State"), sep = ", ", remove = FALSE) %>%
  mutate(ST_ABBREV = state.abb[match(State, state.name)])

# if 'black' or 'white', use moe directly
# if 'other', moe = sqrt(sum of all categories'squared moe values), since the 'other' category includes those
# identified as "native", "asian", "pacific", "other", or "two_or_more"
sub_race_df_clean <- sub_race_df_clean %>%
  group_by(County, ST_ABBREV, race) %>%
  summarise(
    estimate = sum(estimate, na.rm = TRUE),
    moe = if_else(
      race == "other",
      sqrt(sum(moe^2, na.rm = TRUE)),  
      max(moe, na.rm = TRUE)           
    ),
    .groups = "drop"
  ) %>%
  mutate(moe_se = moe / 1.645) %>%
  distinct()%>%
  select(County, ST_ABBREV, estimate, moe, moe_se, race)
```

# Data Cleaning: Ethnicity
```{r}
male_ethn_data <- get_acs(
  geography = "county",
  variables = c("B01001I_002"),
  year = 2023,
  survey = "acs5",
  cache_table = TRUE
)
male_total_data <- get_acs(
  geography = "county",
  variables = c("B01001_002E"),
  year = 2023,
  survey = "acs5",
  cache_table = TRUE
)

male_ethn_data <- male_ethn_data %>%
  rename(
    estimate_hisp = estimate,
    moe_hisp = moe
  )

male_total_data <- male_total_data %>%
  rename(estimate_total = estimate, moe_total = moe)%>%
  inner_join(male_ethn_data %>% select(GEOID, estimate_hisp, moe_hisp), by = "GEOID")%>%
  mutate(estimate_nonhisp = estimate_total - estimate_hisp,
         moe_nonhisp = sqrt(moe_total^2 + moe_hisp^2))

male_total_data <- male_total_data %>%
  select(GEOID, NAME, variable,
         estimate_hisp, moe_hisp,
         estimate_nonhisp, moe_nonhisp) %>%
  pivot_longer(
    cols = c(estimate_hisp, moe_hisp, estimate_nonhisp, moe_nonhisp),
    names_to = c(".value", "ethnicity"),
    names_pattern = "(estimate|moe)_(hisp|nonhisp)"
  )

cols <- c("estimate", "moe")
sub_ethn_df_clean <-male_total_data %>%
  mutate(across(all_of(cols),as.numeric),
         moe_se = moe / 1.645)

sub_ethn_df_clean <- sub_ethn_df_clean %>%
  separate(NAME, into = c("County", "State"), sep = ", ", remove = FALSE) %>%
  mutate(ST_ABBREV = state.abb[match(State, state.name)]) %>%
  select(County, ST_ABBREV, estimate, moe, moe_se, ethnicity)
```
...
params_age_acs <- params_age_acs %>% 
  ...
  arrange(GEOID, Category, factor_lev)
...
params_race_acs <- sub_race_df_clean %>%
  rename(factor_lev = race, est = estimate) %>%
  mutate(Category = "Race") %>%
  left_join(nchscodes, by = c("County" = "CTYNAME", "ST_ABBREV" = "ST_ABBREV")) %>% 
  arrange(GEOID, Category, factor_lev)

params_ethn_acs <- sub_ethn_df_clean %>%
  rename(factor_lev = ethnicity, est = estimate) %>%
...
params_acs_dem <- params_age_acs %>% 
  ...
  bind_rows(params_race_acs)%>%
  bind_rows(params_ethn_acs)%>%
  filter(
...
params_acs_dem <- params_acs_dem %>%
  select(-State, -MOE, -moe)
...
#gss <- read_dta("/Users/karenyu/Library/CloudStorage/OneDrive-SharedLibraries-JohnsHopkins/Meibin Chen - EEE Data/Other/GSS Data/gss7224_r1.dta")
...
# Proportion MSM by Ethnicity and Urbanicity
params_gss_ethn <- svyby(
  ~msm,
  ~ethncat + Urbanicity,
  gss_design,
  svymean,
...
params_gss_race <- params_gss_race %>% 
  mutate(Category = 'racecat') %>%
  rename(factor_lev = racecat)

params_gss_race <- params_gss_race %>% 
  mutate(across(everything(), ~ replace(., . == 0, 1e-6))) %>% 
  as.data.frame()

params_gss_ethn <- params_gss_ethn %>% 
  mutate(Category = 'ethncat') %>%
  rename(factor_lev = ethncat)
...
params_gss_dem <- params_gss_age %>%
  ...
  bind_rows(params_gss_race)%>%
  bind_rows(params_gss_ethn)

#manually set two zero values in params_gss_dem
params_gss_dem <- params_gss_dem %>%
  mutate(msm = ifelse(factor_lev == "black" & Urbanicity == "Nonmetro.Rural", 0.007534475, msm),
         msm = ifelse(factor_lev == "hisp" & Urbanicity == "Nonmetro.Rural", 0.00188556, msm))

params_gss_dem <- params_gss_dem %>% 
  arrange(Category, Urbanicity, factor_lev)
```
...
# Store to read in R
```{r}
save(nchscodes, params_gss, params_gss_dem, params, nchscodes, params_acs_dem, 
     nC, countyid, county, county_to_stateurb, county_urb_code, county_to_urb,
     n_age, n_educ, n_income, n_race, n_ethn, n_urb, 
     ages, educs, incomes, races, ethn, urb_levels,
     gss_p, gss_se, dem_config,
     file = "pop_est_params.RData")
```
