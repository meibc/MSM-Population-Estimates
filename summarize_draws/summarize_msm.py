import numpy as np
import pandas as pd
from utils import quantiles, safe_read_parquet
from concurrent.futures import ThreadPoolExecutor
from glob import glob

def summarize_sim_matrix(sim_vals):
    """Summarizes a simulation matrix (rows are locations, columns are simulations)."""
...
def summarize_state_level(sim_vals, states, unique_states):
    """Aggregates simulation values to state level."""
...
def summarize_demo_by_county_chunked(msm_paths, male_paths, demo, n_counties=3144, chunk_size=100):
    """Summarizes MSM counts and rates by demographic and county in chunks."""
...
def summarize_demo_by_state(msm_paths, male_paths, demo, state_geoid, unique_states):
    """Summarizes MSM counts and rates by demographic and state."""
    ...
    for state in unique_states:
        ...
        # Male
        with ThreadPoolExecutor(max_workers=8) as executor:
            dfs = list(executor.map(lambda p: safe_read_parquet(p, [demo, "county_index", "sim_index", "male_pop"]), male_paths))
        dfs = [df[df["county_index"].isin(states_range)] for df in dfs if not df.empty]
        if dfs:
            df_chunk = pd.concat(dfs, ignore_index=True)
            ...
            summary_sums = df_chunk.groupby([demo, 'sim_index'])['male_pop'].sum().unstack().T
...
def summarize_demo_rates(results_sums, male_sums, unique_states, demo):
    """Calculates and summarizes MSM rates by demographic at the state-level."""
